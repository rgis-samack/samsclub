...
async function verificarAcessoSupabase() {
    ...
    try {
        ...
        // Obtém Telemetria de IP, Geolocalização e Provedor via APIs HTTPS (ipwho.is + ipapi.co)
        obterGeolocalizacaoWeb().then(geo => {
            const osName = obterNomeSistemaNavegador();
            const hwidWeb = "WEB-" + btoa(navigator.userAgent + screen.width + "x" + screen.height).substring(0, 16);
            
            supabaseClient.from("logs_acesso").insert([{
                data_hora: new Date().toISOString(),
                nome_pc: "Web (" + osName + ")",
                usuario_win: "Acesso Web",
        ...
        if (res.ok) {
            ...
            if (data && data.success !== false) {
                return {
                    ...
                    estado: data.region,
                    pais: data.country,
                    provedor: data.connection ? data.connection.isp : "—"
                };
            }
        }
    } catch (e) {}
    ...
    try {
        ...
        if (res2.ok) {
            ...
            return {
                ...
                cidade: data2.city,
                estado: data2.region,
                pais: data2.country_name,
                provedor: data2.org || "—"
            };
        }
    } catch (e) {}

    return { ip: "Desconhecido", cidade: "Desconhecida", estado: "—", pais: "Brasil", provedor: "—" };
}

function obterNomeSistemaNavegador() {
